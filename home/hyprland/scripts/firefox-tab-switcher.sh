...
command -v bt >/dev/null 2>&1 || { notify-send "brotab" "bt not found"; exit 0; }

list=$(bt list 2>/dev/null) \
  || { notify-send "brotab" "no browser client — run 'bt install' and add the extension"; exit 0; }
[ -z "$list" ] && { notify-send "brotab" "no open tabs / client not connected"; exit 0; }
